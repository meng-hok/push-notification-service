...
public class NotificationSendRequest extends CommonRequest {
    ...
    public APNS toApns(){
        APNS apns = new APNS(FileStorageUtil.PUTP8FILEPATH +"/"+agent.getPfilename(),agent.getTeamId(), agent.getFileKey(), agent.getBundleId(), agent.getPush_id(),this.getTitle(), this.getMessage());
    ...
    public FCM toFcm(){
        FCM fcm = new FCM( agent.getAuthorized_key(), agent.getPush_id(),this.getTitle(), this.getMessage());
