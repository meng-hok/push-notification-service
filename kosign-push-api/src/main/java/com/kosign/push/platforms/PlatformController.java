...
public class PlatformController extends SuperController 
{
    ...
    { 
        platformService.insert(request);
    ...
    { 
    	platform.setId(id);
    	Boolean response = platformService.update(platform);
    	
        return response ?  Response.setResponseEntity(HttpStatus.OK) : 
    ...
    { 
    	platform.setId(id);
        Boolean response = platformService.remove(platform);
